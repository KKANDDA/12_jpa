...
@Configuration
@EnableWebSecurity
@EnableGlobalMethodSecurity(prePostEnabled = true) // 컨트롤러 단에서 특정 권한이 있는지 여부를 체크한다.
public class WebSecurityConfig {
    ...
    @Bean
    public WebSecurityCustomizer webSecurityCustomizer() { // 정적 리소스 요청에 대해서는 보안 검사를 하지 않겠다..
    ...
    /**
     * 4. Authentication의 인증 메소드를 제공하는 매니저로 Provider의 인터페이스
     * @return AuthenticationManager */
    @Bean
    public AuthenticationManager authenticationManager() {
        return new ProviderManager(customAuthenticationProvider());
        // ProviderManager는 AuthenticationManager를 구현한 클래스.
        // 여러 AuthenticationProvider를 이용해 인증을 처리함.
    }
    ...
    /**
     * 9. security filter chain 설정 */ // 중요!@!@~!~!!!!!!!!
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        // csrf 보로 비활성화 -> 세션 기반 인증 시 가능한 공격. 토큰방식에서는 의미없음
        http.csrf(csrf -> csrf.disable())
                // JWT 토큰을 사용한느 경우 기본 HTTP 인증 대신 JWT 인증 필터를 사용하도록 지정
