...
public class ProjectionTests {
    ...
    /*
    프로젝션 (projection)
    SELECT 절에 조회할 대상을 지정한느 것을 프로젝션이라고 한다.
    ...
    4. new 명령어를 활용한 프로젝션
        - 다양한 종류의 단순 값들을 DTO로 바로 조회하는 방식으로 new 패키지명.dto명을 쓰면 해당하는 DTO로 바로 반환 받을 수 있다.
    ...
    * */
    ...
    void 단일_엔티티_프로젝션_테스트(){
        ...
        // 엔티티 객체 자체를 조회하는 것이 바로 엔티티 객체 조회.. 객체를 가져오면 엔티티 컨텍스트가 관리해 준다.
        transaction.begin();
    ...
    void 양방향_연관관계_엔티티_프로젝션_테스트(){
        ...
        for (BiDirectionMenu biDirectionMenu : list) {
            System.out.println("biDirectionMenu = " + biDirectionMenu);
        }

        // 메뉴 코드가 3번인 메뉴의 카테고리코드가 10이라서 카테고리코드가 10번인 놈들이 모두 출력됐다.
    }

    @Test
    void 임베디드_프로젝션_테스트(){
        String jpql = "SELECT m.menuIfo FROM embedded_menu m";
        List<MenuIfo> menuIfoList = entityManager.createQuery(jpql, MenuIfo.class).getResultList();

        Assertions.assertNotNull(menuIfoList);
        for (MenuIfo menuIfo : menuIfoList) {
            System.out.println("menuIfo = " + menuIfo);
        }


    /*
    @Embeddable은 데이터베이스에서 중복을 방지하는 역할은 하지 않으며 중복을 방지하려면 테이블에서 유니크 제약 조건을 추가해야 한다.

    @Embeddable은 복합키를 표현할 때 사용하며, 해당 필드나 조합이 유일해야하는 요구사항은 데이터베이스의 제약조건을 통해 해결해야 한다.
    * */
    }

    // 3. 스칼라 타입 프로젝션
    @Test
    void TypeQuery_이용한_스칼라_타입_프로젝션_테스트(){
    ...
    /*
    * 조회하려는 컬럼 값이 2개 이상인 경우, typeQuery로 반환 타입을 지정하지 못한다..
    * 그때 Query를 사용하여 Object[]로 받을 수 있다.
    *
    * 카테고리 코드, 카테고리 네임 반환받아서 출력해 보기*/

    @Test
    void 카테고리코드와카테고리네임을반환받아서출력해보기(){
        String jpql = "SELECT c.categoryCode, c.categoryName FROM category_section03 c";
        List<Object[]> list = entityManager.createQuery(jpql, Object[].class).getResultList();
