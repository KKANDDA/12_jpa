...
// 메소드 단위, 클래스 단위로 작동이 가능하다. 관계 설정 없이 테스트가 가능하다.
public class ProblemOfUsingDirectSQLTests {

    /*
    * Junit = 단위 테스르를 위한 프레임워크이다.
    * 코드의 품질을 개선하고 유지보수성을 높이기 위해 사용한다. 메소드 단위로 테스트를 수행하며,
    ...
    * */
    ...
    /*
    * JDBC API를 이용해 작접 SQL을 다룰 때 발생할 수 있는 문제점
    ...
    * */
    ...
    // 2. SQL에 의존하여 개발
    /*
    * 요구사항 변경에 따라 어플리케이션 수정이 SQL의 수정으로도 이어진다.
    * 이러한 수정에 영향을 미치는 것은 오류를 발생기킬 가능성도 있지만 유지보수성에도 악영향을 미친다.
    ...
    * */
    ...
    // 연관된 객체 문제
    // 연관된 객체가 있는 경우, 객체 간의 관계를 수동으로 매핑해야 하기 때문에 코드가 복잡해 지고 유지보수가 어려워지는 문제가 있다.
    ...
    void testAssociateObject() throws SQLException {
        ...
        for(MenuAndCategory menuAndCategory : menuAndCategoryList){
    ...
    void testEquals() throws SQLException {
        ...
        // 펄스여야 통과, 같지 않다는 뜻.. 같은 내용을 가진 다른 주소..
        // 동등 내용 동등 객체를 지향한다면 오버라이드하는 추가 작업일 필요하다. JPA는 해준다. 동일성을 보장해 준다..
